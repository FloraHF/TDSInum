...
def get_vecs(x):
    D1 = np.concatenate((x[0:2], [0]))
    D2 = np.concatenate((x[2:4], [0]))
    I = np.concatenate((x[4:6], [0]))
...
def phy_to_thtalpha(x):
    ...
    alpha_1 = asin(np.clip(l/np.linalg.norm(D1_I), -1, 1))      # state: alpha_1
    alpha_2 = asin(np.clip(l/np.linalg.norm(D2_I), -1, 1))      # state: alpha_2
...
def phy_to_thtd(x):
    ...
    d1 = max(np.linalg.norm(D1_I), l)
    d2 = max(np.linalg.norm(D2_I), l)
